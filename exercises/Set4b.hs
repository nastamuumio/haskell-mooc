...
countHelper x rest = maybe (rest+1) (const rest) x 
...
maxHelper x rest = if x > rest then x else rest
...
slHelper x rest = (fst rest + x, snd rest + 1)
...
largestHelper x [] = [x]
largestHelper x rest
    | x > maximum rest = [x]
    | x == maximum rest = x : rest
    | otherwise = rest
